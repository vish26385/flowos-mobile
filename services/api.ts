...
// // ✅ Transient retry config (Render sleep / temporary failures)
// const MAX_RETRIES = 3;

// const sleep = (ms: number) => new Promise((r) => setTimeout(r, ms));

// const isTransientError = (error: AxiosError) => {
//   // No response = network / DNS / server down
//   if (!error.response) return true;

//   const status = error.response.status;
//   // Typical transient gateway / overload errors
...
//   // Axios timeout errors often come as code 'ECONNABORTED'
//   // Also handle generic "Network Error"
...
// // 🚀 Handle expired tokens automatically + retry transient failures
// api.interceptors.response.use(
//   (response) => response,
//   async (error: AxiosError) => {
//     const originalRequest = error.config as any;
...
//       );
//     } else {
//       console.log("❌ [RESPONSE] Network/timeout error:", error.message);
//     }

//     // ✅ If token expired (401) -> refresh token flow
//     if (error.response?.status === 401 && !originalRequest?._retry) {
//       if (isRefreshing) {
//         console.log("⏳ [REFRESH] Another refresh in progress, queuing request…");
//         return new Promise(function (resolve, reject) {
...
//     // ✅ Retry transient errors (Render cold start / temporary network)
//     if (originalRequest) {
//       // Avoid retrying refresh endpoint itself (prevents loops)
...
//             1000 * 2 ** (originalRequest._retryCount - 1),
//             4000
//           );

//           console.log(
//             `🔁 [RETRY] ${originalRequest._retryCount}/${MAX_RETRIES} for ${url} after ${delay}ms`
//           );
